...
function(graph, init_node){
  ...
  if(colnames(graph)[3] != "w")  stop("the name of first column has to be w")
  ...
  # Assumption: 
  # Nodes are not ordered: can be (1,3,2)
  # Nodes can be (1,3,5)
  # There is no "dead" nodes, there is no node that can only be visited but not travelled from. 
  
  # Create data frame with distance and name of node
  ...
  # Distance from starting node to itself it set to 0
